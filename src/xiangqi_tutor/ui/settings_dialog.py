...
class SettingsDialog(QDialog):
    ...
    def _engine_page(self) -> QWidget:
        ...
        self.threads.setValue(int(self.store.value("engine/threads", self.settings.engine_threads)))
        self.hash_mb = QSpinBox()
        self.hash_mb.setRange(16, 65536)
        self.hash_mb.setValue(int(self.store.value("engine/hash", self.settings.engine_hash_mb)))
        self.multipv = QSpinBox()
        self.multipv.setRange(1, 10)
        self.multipv.setValue(int(self.store.value("engine/multipv", self.settings.engine_multipv)))
        self.depth = QSpinBox()
        self.depth.setRange(1, 100)
        self.depth.setValue(int(self.store.value("engine/depth", self.settings.engine_depth)))
        self.movetime = QSpinBox()
        self.movetime.setRange(100, 600000)
        self.movetime.setValue(int(self.store.value("engine/movetime", self.settings.engine_movetime_ms)))
        ...
        for label, widget in (
            ...
            ("Hash (MB)", self.hash_mb),
            ("MultiPV", self.multipv),
            ("Depth", self.depth),
            ("MoveTime (ms)", self.movetime),
        ):
    ...
    def _ai_page(self) -> QWidget:
        ...
        self.api_key.setPlaceholderText("留空则保留 Windows 凭据库中的现有密钥")
        self.model = QLineEdit(str(self.store.value("ai/model", self.settings.llm_model)))
        self.temperature = QDoubleSpinBox()
        self.temperature.setRange(0, 2)
        self.temperature.setSingleStep(0.1)
        self.temperature.setValue(float(self.store.value("ai/temperature", self.settings.llm_temperature)))
        self.ai_timeout = QSpinBox()
        self.ai_timeout.setRange(1, 300)
        self.ai_timeout.setValue(int(self.store.value("ai/timeout", self.settings.llm_timeout_seconds)))
        for label, widget in (
            ("API Base URL", self.base_url),
            ("API Key", self.api_key),
            ("Model", self.model),
            ("Temperature", self.temperature),
            ("Timeout (s)", self.ai_timeout),
        ):
            form.addRow(label, widget)
    ...
    def _test_ai(self) -> None:
        ...
        self.ai_test_thread = TutorThread(
            ...
            timeout=float(self.ai_timeout.value()),
            request=request,
        )
    ...
    def _save(self) -> None:
        values = {
            "engine/path": self.engine_path.text().strip(),
            "engine/nnue": self.nnue_path.text().strip(),
            "engine/threads": self.threads.value(),
            "engine/hash": self.hash_mb.value(),
            "engine/multipv": self.multipv.value(),
            "engine/depth": self.depth.value(),
            "engine/movetime": self.movetime.value(),
            "ai/base_url": self.base_url.text().strip(),
            "ai/model": self.model.text().strip(),
            "ai/temperature": self.temperature.value(),
            "ai/timeout": self.ai_timeout.value(),
        }
