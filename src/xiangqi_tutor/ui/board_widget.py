...
class XiangqiBoardWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        left, top, cell = self._geometry()
        board_rect = QRectF(left - cell * .42, top - cell * .42, cell * 8.84, cell * 9.84)
        painter.fillRect(board_rect, QColor("#dfbd7c"))
        painter.setPen(QPen(QColor("#3c2b1f"), max(1.2, cell * .018)))
        for row in range(10):
            y = top + row * cell
        ...
        painter.setFont(QFont("Microsoft YaHei UI", max(12, int(cell * .3)), QFont.Weight.Bold))
        ...
        if self.recommended_move:
            source = self._screen_point(self.recommended_move.source)
            target = self._screen_point(self.recommended_move.target)
            line = QLineF(source, target)
            if line.length() > 0:
                unit = QLineF.fromPolar(cell * .22, line.angle() + 150)
                other = QLineF.fromPolar(cell * .22, line.angle() - 150)
                painter.setBrush(QColor(32, 126, 90, 205))
                painter.setPen(QPen(QColor(32, 126, 90, 205), max(4, cell * .08), Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        ...
        for square, piece in self.game.position.pieces():
            ...
            painter.setPen(QPen(color, max(2, cell * .04)))
            painter.drawEllipse(center, radius, radius)
            painter.setPen(color)
            painter.setFont(QFont("Microsoft YaHei UI", max(14, int(cell * .43)), QFont.Weight.Bold))
            painter.drawText(QRectF(center.x()-radius, center.y()-radius, radius*2, radius*2), Qt.AlignmentFlag.AlignCenter, PIECE_TEXT[(piece.side, piece.kind)])

        if self.selected:
            center = self._screen_point(self.selected)
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.setPen(QPen(QColor("#1d7f5f"), max(3, cell * .06)))
            painter.drawEllipse(center, cell * .46, cell * .46)
            painter.setBrush(QColor(30, 140, 95, 155))
            painter.setPen(Qt.PenStyle.NoPen)
            for target in self.game.legal_targets(self.selected):
                point = self._screen_point(target)
                painter.drawEllipse(point, cell * .11, cell * .11)
        if self.game.position.is_in_check(self.game.position.side_to_move):
            ...
            if general:
                ...
                painter.setBrush(Qt.BrushStyle.NoBrush)
                painter.setPen(QPen(QColor("#d02020"), max(4, cell * .08)))
                painter.drawEllipse(point, cell * .49, cell * .49)
