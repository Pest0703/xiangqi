    ...
    def run(self) -> None:
        try:
            ...
            self.service.configure(
                threads=self.params["threads"], hash_mb=self.params["hash_mb"], multipv=self.params["multipv"], nnue=self.params["nnue"]
            )
...
class TutorThread(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            cache_key = self.params["cache_key"]
            if database and cache_key:
                cached = database.get_tutor_cache(cache_key)
                if cached:
            ...
            result = asyncio.run(provider.complete(self.params["request"]))
            if database and cache_key:
                database.put_tutor_cache(cache_key, self.params["fen"], self.params["model"], result)
            self.completed.emit(result)
        except Exception as exc:
            self.failed.emit(str(exc))
        finally:
